...
(function($) {
    ...
    const MapPicker = {
        ...
        init() {
            ...
            this.mapContainer = $('.division-coordinates-map:visible')[0];
         ...
         */
        debugLog(...args) {
            if (window.wpAgencyMapSettings?.debug) {
    ...
    $(document).ready(() => {
        // Handle modal events
        $(document).on('division:modalOpened', () => {
            setTimeout(() => {
                window.MapPicker.init();
            }, 300);
        });

        $(document).on('division:modalFullyOpen', () => {
            if (window.MapPicker.map) {
                window.MapPicker.map.invalidateSize();
            }
        });

        $(document).on('division:modalClosed', () => {
