/**
 * Agency Membership JavaScript Handler
 *
 * @package     WP_Agency
 ...
 * @author      arisciwek
 *
 * Path: /wp-agency/assets/js/agency/agency-membership.js
 *
 * Description: JavaScript handler untuk agency membership tab.
...
(function($) {
    ...
    const AgencyMembership = {
        ...
        init() {
            this.bindEvents();
            if (window.Agency && window.Agency.currentId) {
                this.currentId = window.Agency.currentId;
		...
		loadMembershipStatus() {
		    console.log('Loading membership status for agency:', this.currentId);
		    ...
		    $.ajax({
		        ...
		        success: (response) => {
		            ...
		            if (response.success) {
		                ...
		                AgencyToast.error(response.data.message || 'Failed to load membership data');
		            }
		        },
		        error: (xhr, status, error) => {
		            console.error('AJAX error:', {xhr, status, error});
		            AgencyToast.error('Failed to load membership status');
		        }
		    });
		},
    ...
    $(document).ready(() => {
        window.AgencyMembership = AgencyMembership;
