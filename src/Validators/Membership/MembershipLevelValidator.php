 ...
 * @author      arisciwek
 *
 * Path: /wp-agency/src/Validators/Membership/MembershipLevelValidator.php
 *
...
namespace WPAgency\Validators\Membership;

class MembershipLevelValidator {
    private $model;

    public function __construct() {
        $this->model = new \WPAgency\Models\Membership\MembershipLevelModel();
    }
    ...
    public function validate_level($data, $id = null) {
        ...
        // Required fields
        if (empty($data['name'])) {
            $errors[] = __('Level name is required.', 'wp-agency');
        }

        // Name length
        if (strlen($data['name']) > 50) {
            $errors[] = __('Level name cannot exceed 50 characters.', 'wp-agency');
        }

        // Check slug uniqueness (except for current level when editing)
        if ($this->model->exists_by_slug($data['slug'], $id)) {
            $errors[] = __('A level with this name already exists.', 'wp-agency');
        }

        // Price validation
        if (!isset($data['price_per_month']) || $data['price_per_month'] < 0) {
            $errors[] = __('Price must be 0 or greater.', 'wp-agency');
        }

        // Trial period validation
        if (!empty($data['is_trial_available'])) {
            if (empty($data['trial_days']) || $data['trial_days'] < 0) {
                $errors[] = __('Trial days must be greater than 0 when trial is enabled.', 'wp-agency');
            }
        }

        // Grace period validation
        if (!isset($data['grace_period_days']) || $data['grace_period_days'] < 0) {
            $errors[] = __('Grace period days must be 0 or greater.', 'wp-agency');
        }
    ...
    private function validate_capabilities($capabilities) {
        ...
        if (!is_array($capabilities)) {
            $errors[] = __('Invalid capabilities format.', 'wp-agency');
            return $errors;
        }
        ...
        foreach ($required_sections as $section) {
            if (!isset($capabilities[$section]) || !is_array($capabilities[$section])) {
                $errors[] = sprintf(__('Missing or invalid %s section in capabilities.', 'wp-agency'), $section);
            }
        }
        ...
        if (isset($capabilities['limits'])) {
            foreach ($capabilities['limits'] as $key => $limit) {
                if (!isset($limit['value']) || !is_numeric($limit['value'])) {
                    $errors[] = sprintf(__('Invalid value for limit: %s', 'wp-agency'), $key);
                }
            }
        }
        ...
        if (isset($capabilities['features'])) {
            foreach ($capabilities['features'] as $key => $feature) {
                if (!isset($feature['value']) || !is_bool($feature['value'])) {
                    $errors[] = sprintf(__('Invalid value for feature: %s', 'wp-agency'), $key);
                }
            }
        }
    ...
    public function validate_delete($level_id) {
        ...
        if (!$this->model->get_level($level_id)) {
            $errors[] = __('Level not found.', 'wp-agency');
        }

        // Check if level is in use
        if ($this->model->is_level_in_use($level_id)) {
            $errors[] = __('Cannot delete level because it is currently in use by one or more agencies.', 'wp-agency');
        }
