 ...
 * @author      arisciwek
 *
 * Path: /wp-agency/src/Integrations/EmployeeAccessFilter.php
 *
 * Description: Specific filtering untuk Employee DataTable.
 *              Filters employees based on user role and assignment.
 *              Pattern inspired by wp-customer EmployeeAccessFilter.
 *
 * Hook: wpdt_datatable_agency_employees_where
 *
 * Role-Based Access:
...
class EmployeeAccessFilter {
    ...
    private function register_hooks(): void {
        add_filter(
            'wpdt_datatable_agency_employees_where',
            [$this, 'filter_employees_by_role'],
            10,
            3
        );
    }
     ...
     */
    public function filter_employees_by_role($where_conditions, $request_data, $model): array {
        ...
        // Get accessible employee IDs (all roles are filtered by division)
        $accessible_ids = $this->relation_model->get_accessible_entity_ids('employee', $user_id);
        ...
        $where_conditions[] = "e.id IN ({$ids_string})";
