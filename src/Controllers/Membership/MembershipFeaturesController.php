 ...
 * @author      arisciwek
 *
 * Path: /wp-agency/src/Controllers/Membership/MembershipFeaturesController.php
 *
 ...
 * - AgencyCacheManager for caching
 * - WordPress AJAX API
...
namespace WPAgency\Controllers\Membership;
...
class MembershipFeaturesController {
    ...
    public function __construct() {
        ...
        $this->cache_manager = new AgencyCacheManager();
    }
        /**
        ...
        public function handle_get_group() {
            try {
                ...
                check_ajax_referer('wp_agency_membership_level', 'nonce');
                ...
                if (!$group_data) {
                    throw new \Exception(__('Group tidak ditemukan', 'wp-agency'));
                }
        ...
        public function handle_save_group() {
            try {
                ...
                if ($this->model->group_exists_by_slug($group_data['slug'], $group_id)) {
                    throw new \Exception(__('Group dengan slug tersebut sudah ada', 'wp-agency'));
                }

                // Save ke database
                $result = $this->model->save_feature_group($group_id, $group_data);
                if (!$result) {
                    throw new \Exception(__('Gagal menyimpan data group', 'wp-agency'));
                }
                ...
                wp_send_json_success([
                    'message' => $group_id ? 
                        __('Group berhasil diupdate', 'wp-agency') : 
                        __('Group baru berhasil ditambahkan', 'wp-agency'),
                    'id' => $result
                ]);
        ...
        public function handle_delete_group() {
            try {
                ...
                if ($this->model->group_has_features($group_id)) {
                    throw new \Exception(__('Group tidak dapat dihapus karena masih memiliki features', 'wp-agency'));
                }

                // Hapus group
                $result = $this->model->delete_feature_group($group_id);
                if (!$result) {
                    throw new \Exception(__('Gagal menghapus group', 'wp-agency'));
                }
                ...
                wp_send_json_success([
                    'message' => __('Group berhasil dihapus', 'wp-agency')
                ]);
    ...
    public function handle_get_feature() {
        try {
            ...
            if (!$feature) {
                throw new \Exception(__('Feature not found.', 'wp-agency'));
            }
	...
	public function handle_save_feature() {
        try {
            ...
            if (!$result) {
                throw new \Exception(__('Failed to save feature.', 'wp-agency'));
            }
            ...
            $message = $id ? __('Feature updated successfully.', 'wp-agency') 
                         : __('Feature added successfully.', 'wp-agency');
    ...
    public function handle_delete_feature() {
        try {
            ...
            if (!$result) {
                throw new \Exception(__('Failed to delete feature.', 'wp-agency'));
            }
            ...
            wp_send_json_success([
                'message' => __('Feature deleted successfully.', 'wp-agency')
            ]);
    ...
    public function getFeatureGroups() {
        ...
        if ($groups === null) {
            ...
            $results = $wpdb->get_results("
                ...
                FROM {$wpdb->prefix}app_agency_membership_features
                WHERE status = 'active'
    ...
    public function handle_save_level() {
        try {
            ...
            if (!$result) {
                throw new \Exception(__('Failed to save membership level.', 'wp-agency'));
            }
            ...
            wp_send_json_success([
                'message' => $id ? 
                    __('Level updated successfully.', 'wp-agency') : 
                    __('Level added successfully.', 'wp-agency'),
                'id' => $result
            ]);
