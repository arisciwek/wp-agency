 ...
 * @author      arisciwek
 *
 * Path: /wp-agency/src/Controllers/Membership/AgencyMembershipController.php
 *
 * Description: Controller untuk mengelola operasi terkait membership agency:
...
class AgencyMembershipController {
	...
	public function saveMembershipLevel() {
	   try {
	       ...
	       $levelModel = new AgencyMembershipLevelModel();
	...
	public function createMembership(array $membership_data): int {
	    try {
	        ...
	        if (empty($membership_data['agency_id']) || empty($membership_data['level_id'])) {
	        ...
	        $this->cache->delete('agency_membership', $membership_data['agency_id']);
	        $this->cache->invalidateDataTableCache('membership_list', [
	            'agency_id' => $membership_data['agency_id']
	        ]);
	...
	public function getMembershipStatus() {
	    try {
	        ...
	        // Get membership data
	        $membership = $this->membership_model->findByAgency($agency_id);
	        if (!$membership) {
    ...
    public function upgradeMembership() {
        try {
            ...
            if (!$agency_id || !$new_level_id) {
                throw new \Exception('Invalid parameters');
            }

            // Validate upgrade possibility
            if (!$this->membership_model->canUpgrade($agency_id, $new_level_id)) {
			...
			$this->cache->delete('agency_active_employee_count', $agency_id);
    ...
    public function extendMembership() {
        try {
            ...
            $updated = $this->membership_model->findByAgency($agency_id);
