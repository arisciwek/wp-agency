 ...
 * @author      arisciwek
 *
 * Path: /wp-agency/src/Controllers/Membership/MembershipLevelController.php
 *
 ...
 * - AgencyCacheManager untuk caching
 * - WordPress AJAX API
 *
 * Changelog:
 * 1.0.0 - 2024-02-11
...
use WPAgency\Validators\Membership\MembershipLevelValidator;
...
class MembershipLevelController {
    ...
    public function handle_get_level() {
        try {
            ...
            if (!$level) {
                throw new \Exception(__('Level tidak ditemukan.', 'wp-agency'));
            }
    ...
    public function handle_save_level() {
        try {
            ...
            try {
                ...
                throw new \Exception(__('Failed to process membership data.', 'wp-agency'));
            }
            ...
            if (!$this->validateCapabilitiesStructure($capabilities)) {
                throw new \Exception(__('Invalid capabilities structure.', 'wp-agency'));
            }
    ...
    public function handle_delete_level() {
        try {
            ...
            if (!$result) {
                throw new \Exception(__('Failed to delete membership level.', 'wp-agency'));
            }
            ...
            wp_send_json_success([
                'message' => __('Level deleted successfully.', 'wp-agency')
            ]);
