 ...
 * @author      arisciwek
 *
 * Path: /wp-agency/src/Models/Membership/MembershipFeatureModel.php
 *
...
class MembershipFeatureModel {
    ...
    public function __construct() {
        ...
        $this->table = $wpdb->prefix . 'app_agency_membership_features';  // Inisialisasi table
        $this->table_groups = $wpdb->prefix . 'app_agency_membership_feature_groups';
        $this->table_features = $wpdb->prefix . 'app_agency_membership_features';
    }
    ...
    public function get_feature_group($group_id) {
        return $this->wpdb->get_row($this->wpdb->prepare(
            "SELECT * FROM {$this->wpdb->prefix}app_agency_membership_feature_groups 
    ...
    public function group_exists_by_slug($slug, $exclude_id = null) {
        $query = "SELECT COUNT(*) FROM {$this->wpdb->prefix}app_agency_membership_feature_groups 
    ...
    public function save_feature_group($id, $data) {
        if ($id > 0) {
            return $this->wpdb->update(
                $this->wpdb->prefix . 'app_agency_membership_feature_groups',
                $data,
                ['id' => $id]
            ) !== false ? $id : false;
        } else {
    ...
    public function group_has_features($group_id) {
        return (bool) $this->wpdb->get_var($this->wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->wpdb->prefix}app_agency_membership_features 
             WHERE group_id = %d AND status = 'active'",
            $group_id
        ));
    }
    ...
    public function get_group_capability_mapping() {
        ...
        if ($mapping === false) {
            $results = $wpdb->get_results("
                ...
                FROM {$wpdb->prefix}app_agency_membership_feature_groups 
                WHERE status = 'active'
            ", ARRAY_A);
