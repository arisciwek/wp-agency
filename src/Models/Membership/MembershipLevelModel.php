 ...
 * @author      arisciwek
 *
 * Path: /wp-agency/src/Models/Membership/MembershipLevelModel.php
 *
...
class MembershipLevelModel {
    ...
    public function getActiveGroups() {
        global $wpdb;
        $table = $wpdb->prefix . 'app_agency_membership_features';
    ...
    public function getGroupMapping() {
        ...
        $custom_mapping = get_option('wp_agency_group_mapping', []);
    ...
    /**
     * Check if level is in use by any agencies
     */
    public function is_level_in_use($level_id) {
        $agencies_table = $this->wpdb->prefix . 'app_agencies';
        
        return (bool) $this->wpdb->get_var($this->wpdb->prepare(
            "SELECT COUNT(*) FROM {$agencies_table} 
    ...
    /**
     * Get total active agencies for a level
     */
    public function get_active_agencies_count($level_id) {
        $agencies_table = $this->wpdb->prefix . 'app_agencies';
