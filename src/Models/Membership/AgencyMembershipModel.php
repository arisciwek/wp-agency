 ...
 *
 * @package     WP_Agency
 * @subpackage  Models/Agency/AgencyMembershipLevel
 * @version     1.0.0
 * @author      arisciwek
 *
 * Path: /wp-agency/src/Models/Membership/AgencyMembershipLevelModel.php
 *
 * Description: Model untuk mengelola data agency membership level secara dinamis.
 *              Menggantikan sistem agency_membership yang sebelumnya hardcoded.
 *              Menggunakan database table untuk menyimpan konfigurasi level.
 *              Terintegrasi dengan caching untuk optimasi performa.
 *              
 * Database Table: {prefix}app_agency_membership_levels
 * Fields:
 * - id             : Primary key
 ...
 * 
 * Dependencies:
 * - WPAgency\Cache\AgencyCacheManager untuk caching
 ...
 * - Group: wp_agency
 * - Keys:
 *   - agency_membership_levels_all: Semua level yang aktif
 *   - agency_membership_level_{id}: Data single level
 *   - user_agency_membership_level_{user_id}: Level ID untuk user
 *
 * Migration Note:
 * Untuk migrasi dari sistem lama:
 * 1. Baca data dari wp_agency_membership_settings option
 * 2. Convert ke format database baru
 * 3. Maintain backward compatibility selama transisi
 *
 * Usage Example:
 * $agencyMembershipLevel = new AgencyMembershipLevelModel();
 * $levels = $agencyMembership->getAllLevels();
 * $capabilities = $agencyMembership->getLevelCapabilities($level_id);
 * $has_access = $agencyMembership->hasCapability($user_id, 'can_export');
 * 
 * Changelog:
 * 1.0.0 - 2024-02-08
 * - Initial version
...
namespace WPAgency\Models\Membership;
...
class AgencyMembershipModel {
    private $table;
    private $cache;
    private $membership_settings_key = 'wp_agency_membership_settings';
    
    public function __construct() {
        global $wpdb;
        $this->table = $wpdb->prefix . 'app_agency_membership_levels';
        $this->cache = new AgencyCacheManager();
    }

    /**
     * Get all agency_membership levels
     */
    public function getAllLevels(): array {
        // Check cache first
        $cached = $this->cache->get('agency_membership_levels', 'all');
        if ($cached !== null) {
            return $cached;
        }
        ...
        // Cache for 5 minutes
        $this->cache->set('agency_membership_levels', $levels, 300, 'all');
        
        return $levels;
    }

    /**
     * Get single agency_membership level
     */
    public function getLevel(int $id): ?object {
        // Check cache
        $cached = $this->cache->get('agency_membership_level', $id);
        if ($cached !== null) {
            return $cached;
        }
        ...
        if ($level) {
            // Cache for 5 minutes
            $this->cache->set('agency_membership_level', $level, 300, $id);
        }
    ...
    /**
     * Create new agency_membership level
     */
    ...
    /**
     * Update agency_membership level
     */
    ...
    /**
     * Delete agency_membership level
     */
    ...
    /**
     * Get agency_membership level capabilities
     */
    ...
    /**
     * Check if user has capability in their agency_membership level
     */
    ...
    /**
     * Get user's current agency_membership level
     */
    public function getUserMembershipLevel(int $user_id): ?int {
        // Check cache
        $cached = $this->cache->get('user_agency_membership_level', $user_id);
        if ($cached !== null) {
            return $cached;
        }
        ...
        $level_id = $wpdb->get_var($wpdb->prepare("
            SELECT agency_membership_level_id
        ...
        if ($level_id) {
            // Cache for 5 minutes
            $this->cache->set('user_agency_membership_level', (int)$level_id, 300, $user_id);
        }
    ...
    /**
     * Clear all agency_membership-related cache
     */
    private function clearCache(): void {
        $this->cache->delete('agency_membership_levels', 'all');
        // Could also clear other agency_membership-related caches here
    }
    ...
    public function getAvailableCapabilities(): array {
        return [
            'can_add_staff' => __('Dapat menambah staff', 'wp-agency'),
            'can_export' => __('Dapat export data', 'wp-agency'),
            'can_bulk_import' => __('Dapat bulk import', 'wp-agency'),
            'can_manage_departments' => __('Dapat mengelola departemen', 'wp-agency'),
            'can_view_reports' => __('Dapat melihat laporan', 'wp-agency'),
            'can_manage_settings' => __('Dapat mengatur pengaturan', 'wp-agency')
        ];
    }

    /**
     * Get agency_membership level limits
     */
    ...
    public function getMembershipData(int $agency_id): array {
        ...
        $agency = $this->getAgencyLevel($agency_id);
    ...
    /**
     * Get agency's membership level
     */
    private function getAgencyLevel(int $agency_id): ?object {
        global $wpdb;
	 ...
	 * Matches schema from AgencyMembershipLevelsDB
	 */
    /*
