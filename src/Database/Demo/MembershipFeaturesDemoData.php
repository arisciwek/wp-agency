 ...
 * @author      arisciwek
 *
 * Path: /wp-agency/src/Database/Demo/MembershipFeaturesDemoData.php
 * 
...
class MembershipFeaturesDemoData extends AbstractDemoData {
    ...
    protected function validate(): bool {
        try {
            ...
            $table_exists = $this->wpdb->get_var(
                "SHOW TABLES LIKE '{$this->wpdb->prefix}app_agency_membership_features'"
            );
    ...
    private function insertDefaultGroups(): array {
        global $wpdb;
        $table_name = $wpdb->prefix . 'app_agency_membership_feature_groups';
    ...
    private function insertDefaultFeatures(array $group_ids): void {
        global $wpdb;
        $table_name = $wpdb->prefix . 'app_agency_membership_features';
        $current_user_id = get_current_user_id();
    ...
    private function clearExistingData(): void {
        try {
            ...
            $this->wpdb->query(
                "DELETE FROM {$this->wpdb->prefix}app_agency_membership_features 
                 WHERE id > 0"
            );

            // Reset auto increment
            $this->wpdb->query(
                "ALTER TABLE {$this->wpdb->prefix}app_agency_membership_features 
                 AUTO_INCREMENT = 1"
            );
