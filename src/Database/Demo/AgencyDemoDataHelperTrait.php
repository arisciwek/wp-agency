 ...
 * @author      arisciwek
 *
 * Path: /wp-agency/src/Database/Demo/AgencyDemoDataHelperTrait.php
 *
 ...
 * 1.0.0 - 2024-01-27
 * - Initial version
 * - Focused on agency-specific helper methods
...
trait AgencyDemoDataHelperTrait {
    /**
     * Generate division location
     */
    private function generateDivision($agency_id, $type = 'cabang'): void {
        ...
        $agency = $this->agencyModel->getById($agency_id);
        
        $division_data = [
            'agency_id' => $agency_id,
            'code' => $this->divisionModel->generateDivisionCode(),
            'name' => $type === 'pusat' ? 
                     $agency->name . ' Kantor Pusat' : 
                     $agency->name . ' Division ' . $regency_name,
            'type' => $type,
            ...
            'email' => sprintf('%s.%s@%s',
                      $type,
                      strtolower(str_replace([' ', '.'], '', $agency->name)),
                      $domains[array_rand($domains)]),
            'provinsi_id' => $provinsi_id,
            'regency_id' => $regency_id,
            'user_id' => $this->user_ids[$agency_id],
            'created_by' => $this->user_ids[$agency_id],
            'status' => 'active'
        ];

        $division_id = $this->divisionModel->create($division_data);
        if (!$division_id) {
            throw new \Exception("Failed to create division for agency: {$agency_id}");
        }
     ...
     * This can be enabled via settings or WP_AGENCY_DEVELOPMENT constant
     * 
    ...
    public function isDevelopmentMode(): bool {
        ...
        return (isset($dev_settings['enable_development']) && $dev_settings['enable_development']) 
               || (defined('WP_AGENCY_DEVELOPMENT') && WP_AGENCY_DEVELOPMENT);
    }
    /*
