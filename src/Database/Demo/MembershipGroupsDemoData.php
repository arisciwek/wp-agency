 ...
 * @author      arisciwek
 *
 * Path: /wp-agency/src/Database/Demo/MembershipGroupsDemoData.php
 * 
...
class MembershipGroupsDemoData extends AbstractDemoData {
    ...
    protected function validate(): bool {
        try {
            ...
            $table_exists = $this->wpdb->get_var(
                "SHOW TABLES LIKE '{$this->wpdb->prefix}app_agency_membership_feature_groups'"
            );
    ...
    private function clearExistingData(): void {
        try {
            ...
            $this->wpdb->query(
                "DELETE FROM {$this->wpdb->prefix}app_agency_membership_feature_groups 
                 WHERE id > 0"
            );

            // Reset auto increment untuk ID yang konsisten
            $this->wpdb->query(
                "ALTER TABLE {$this->wpdb->prefix}app_agency_membership_feature_groups 
                 AUTO_INCREMENT = 1"
            );
